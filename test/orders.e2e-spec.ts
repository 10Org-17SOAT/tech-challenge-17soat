...
describe('Orders (e2e)', () => {
  ...
  beforeEach(() => pool.query('DELETE FROM orders'));
  ...
  const orderBody = (res: request.Response) => res.body as OrderResponse;
  ...
  describe('POST /orders', () => {
    it('creates an order in status received with defaults', async () => {
      const body = orderBody(await http().post('/orders').send({}).expect(201));
    ...
    it('ignores unknown fields', async () => {
      const body = orderBody(
        await http().post('/orders').send({ foo: 'bar' }).expect(201),
      );
  ...
  describe('GET /orders', () => {
    it('paginates and filters by status', async () => {
      for (let i = 0; i < 3; i++) {
        await http().post('/orders').send({}).expect(201);
      }
      ...
      const all = pageBody(await http().get('/orders').expect(200));
      expect(all.total).toBe(4);

      const filtered = pageBody(
        await http().get('/orders?status=in_diagnosis').expect(200),
      );
    ...
    it('rejects invalid params', async () => {
      await http().get('/orders?page=0').expect(400);
      await http().get('/orders?status=unknown').expect(400);
    });
  });

  describe('GET /orders/:id', () => {
    it('returns the order or 404', async () => {
      const created = orderBody(await http().post('/orders').send({}));
      const body = orderBody(
        await http().get(`/orders/${created.id}`).expect(200),
      );
      expect(body.id).toBe(created.id);

      await http()
        .get('/orders/6f2c8e0a-0b0e-4f6e-9e1e-000000000000')
        .expect(404);
      await http().get('/orders/not-a-uuid').expect(400);
    });
  });
  ...
  describe('PATCH /orders/:id', () => {
    ...
    it('returns 404 for unknown id', async () => {
      await http()
        .patch('/orders/6f2c8e0a-0b0e-4f6e-9e1e-000000000000')
        .send({ notes: 'x' })
        .expect(404);
    });
  });
  ...
  describe('DELETE /orders/:id', () => {
    it('soft deletes an order in received', async () => {
      ...
      await http().delete(`/orders/${id}`).expect(204);
      await http().get(`/orders/${id}`).expect(404);
      await http().delete(`/orders/${id}`).expect(404);

      const list = pageBody(await http().get('/orders').expect(200));
      expect(list.total).toBe(0);

      const { rows } = await pool.query<{ deleted_at: Date | null }>(
        'SELECT deleted_at FROM orders WHERE order_id = $1',
        [id],
      );
    ...
    it('refuses to delete once past received with 409', async () => {
      ...
      await http().delete(`/orders/${created.id}`).expect(409);
    });
  });
});
